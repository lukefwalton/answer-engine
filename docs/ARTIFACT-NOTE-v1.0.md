...
A disclosure belongs first, because this is the one section of the note whose evidence is not reproducible: the production corpus is private, its size and composition are withheld, and the figures below are illustrative ratios from a system the reader cannot run. What is public and checkable is the method. Every cost-reducing change was admitted or rejected by the same gold suite that governs grounding and refusal — `eval/gold.yaml`, in this repository — under the rule of Section 4 that a regression is repaired in the corpus, the scoring, or the prompt and never by special-casing a query. The numbers exemplify; the discipline carries the claim, and the discipline is in the artifact.

The cost concentrates almost entirely in one object — the embedding index, in both its in-memory footprint and the latency of making it available to a stateless serving instance — and three levers reduce it, each a choice made affordable here rather than the only choice or a guarantee of affordability in general, none of which alters the contract of Section 3.

**Embedding dimension.** The dimensionality of the embedding is a tunable cost: models trained with Matryoshka representation learning produce vectors that can be truncated and renormalized after the fact with graceful quality decay, so the index can be rebuilt at a lower dimensionality and the query embedded to match. Whether to do so is decided against the gold suite, not by intuition — a dimension that fails an enumerated-recall or a refusal case is rejected — and the homogeneity the change requires is itself a checked invariant: the stored (model, dimensions) pair is the index's identity, the query is embedded at that same dimensionality, and a store that mixed dimensions fails fast rather than comparing incomparable vectors. The saving is real — in this deployment roughly threefold, from 3072 to 1024 dimensions, for a small and measured quality cost — but the load-bearing point is that it was admitted by the gate, which puts a cost lever under the same authority that certifies soundness. The scaling decision is, in that exact sense, answerable.

**Wire format.** This cost is absent from the bounded implementation, where the index is read in process; it appears only once the index must be transported to a stateless serving instance, where its serialized size becomes a cold-start latency paid by every cold instance. Embedding vectors dominate that serialized form, and quantizing the published vectors to one signed byte per dimension — symmetric, with a per-vector scale — reduces each vector by roughly an order of magnitude, and the serialized bundle by somewhat less, since the ids and metadata it also carries do not quantize. Two facts make this admissible, and they differ in kind. The first is exact: cosine similarity normalizes by vector norm, so a positive per-vector scale cancels from the comparison entirely — the ranking is invariant to it as a matter of algebra, and one can score against the quantized bytes without restoring the scale at all. The second is not exact: int8 rounding perturbs each vector's direction and can reorder near-ties, so its harmlessness is not proven but measured — the quantized index is checked against the gold suite and accepted only if it holds (here, rank correlation above 0.99, then a passing eval). That split is the same epistemic move the rest of the note makes: what can be guaranteed is guaranteed, here by algebra rather than by a type, and what cannot is gated by the eval rather than asserted. The full-precision vectors remain the source of truth in memory and on disk, so quantization is a transport encoding rather than a lossy store, and the wire format is versioned so that a mismatch between code and stored blob fails loudly instead of misreading bytes.
...
Across this deployment the index footprint and cold-start cost were cut several-fold — the figure is the least of it, and the least verifiable. The claim worth keeping is methodological and public: every one of these optimizations was admitted by the gold suite rather than by a judgment about what ought to be harmless, so the same gate that owns soundness also adjudicated the cost choices made in its name. That is the strongest sense in which answerability here is operative rather than nominal — it governs not only what the system may answer but how the system that answers may be tuned. These notes establish feasibility and that discipline for a single system; they are not a general cost model, and a controlled study across systems and corpora remains future work.
