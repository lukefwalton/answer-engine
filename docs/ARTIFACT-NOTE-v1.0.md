...
Section 6 holds that what grows with scale is the cost of operating the pattern, not its applicability, and leaves the economic question empirical. This section does not close that question — a single deployment cannot — but answers it once, by example: notes from the production system behind "Ask the Archive," which operates the same pattern over a corpus several orders of magnitude larger than the bundled example. The levers below are one set of choices that made operation affordable here, not the only ones, and not a guarantee of affordability in general. Consistent with the disclaimer in Section 5, these are notes rather than a benchmark or a cost model: they report where the operating cost concentrates and, more to the point, how each lever for reducing it was decided. Figures are given as ratios; the production corpus's size and composition are withheld as properties of a private archive. The cost concentrates almost entirely in one object — the embedding index, both its footprint in memory and the latency of making it available to a stateless serving instance — and three levers reduce it, none of which alters the contract of Section 3.

**Embedding dimension.** Models trained with Matryoshka representation learning produce vectors that can be truncated and renormalized after the fact with graceful quality decay, so the index can be re-embedded at a lower dimensionality, trading a small and measured retrieval-quality decrement for a roughly proportional reduction in size — on the order of threefold from 3072 to 1024 dimensions. The choice is not made by intuition but against the gold suite: a dimension that fails an enumerated-recall or a refusal case is rejected. This is the load-bearing observation of the section, because it places the cost lever under the same gate that certifies grounding and refusal. The scaling decision is itself answerable, governed by the rule stated in Section 4 — that a regression is repaired in the corpus, the scoring, or the prompt, never by special-casing the failing query.

**Wire format.** This cost is absent from the bounded implementation, where the index is read in process; it appears only once the index must be transported to a stateless serving instance, where its serialized size becomes a cold-start latency paid by every cold instance. Embedding vectors dominate that serialized form. Quantizing the published vectors to one signed byte per dimension — symmetric, with a per-vector scale — reduces the transported size by roughly an order of magnitude while preserving cosine rank, because the similarity score recomputes each vector's norm and the per-vector scale therefore cancels out of the comparison; the encoding spends directional precision only, never magnitude calibration. The full-precision vectors remain the source of truth in memory and on disk, so quantization is a transport encoding rather than a lossy store, and the wire format is versioned so that a mismatch between code and stored blob fails loudly instead of misreading bytes. The admissibility criterion here is methodological rather than merely pragmatic: the optimization is permitted precisely because it is provably inert with respect to the ranking on which the gate depends.

**Corpus geometry.** Indexing passages rather than whole documents — necessary once a document is long enough that a single embedding dilutes its topical center — makes index size scale with passage count, that is, linearly in the corpus's decomposed size rather than in its document count. The granularity of the passage window trades index size against retrieval precision and is, once again, chosen against the gold suite rather than assumed.

Across this deployment the index footprint and the cold-start cost were each reduced by roughly an order of magnitude through these levers, and in every case the change was admitted by the evaluation suite rather than by a judgment about what ought to be harmless. This is the empirical content of the claim in Section 6 that what grows with scale is the cost of operating the pattern: at a real corpus size the cost was tractable, and — the point worth keeping — the same gate that owns soundness also adjudicated the optimizations made in its name. The notes establish feasibility and the governing discipline for a single system; they are not a general cost model, and a controlled study across systems and corpora remains future work.
