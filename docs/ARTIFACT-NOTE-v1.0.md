...
Section 6 holds that what grows with scale is the cost of operating the pattern, not its applicability, and leaves the economic question empirical. This section records notes from one deployment that operates the same pattern — the production system behind "Ask the Archive" — over a corpus several orders of magnitude larger than the bundled example. Consistent with the disclaimer in Section 5, these are notes rather than a benchmark or a cost model: they report where the operating cost concentrates and, more to the point, how each lever for reducing it was decided. Figures are given as ratios; the production corpus's size and composition are withheld as properties of a private archive. The cost concentrates almost entirely in one object — the embedding index, both its footprint in memory and the latency of making it available to a stateless serving instance — and three levers reduce it, none of which alters the contract of Section 3.
