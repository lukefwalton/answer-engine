...
The footprint and cold-start cost fell several-fold, but that figure is the least of it and the least verifiable. What matters is that the gate which owns soundness also adjudicated every cost cut made in its name: answerability here governs not just what the system may answer but how it may be tuned. These are notes from a single system, not a general cost model; a controlled study across systems and corpora remains future work.
