...
A disclosure belongs first. The production corpus is private — you can read these figures but cannot run it, so they are real, not reproducible: 380 songs, 210 podcast episodes (~123 hours), and assorted public and private writings, indexed as 10,554 embedding vectors — 9,777 private transcript passages and 777 public records. What is public and checkable is the method — every cost-reducing change was admitted or rejected by the same gold suite that governs grounding and refusal (`eval/gold.yaml`), under Section 4's rule against special-casing a failing query.
...
That cut — int8 alone, ~616 MB to ~53 MB — is the least of what matters here. What matters is that the gate which owns soundness also adjudicated every cost choice made in its name: answerability here governs not just what the system may answer but how it may be tuned. These are notes from a single system, not a general cost model; a controlled study across systems and corpora remains future work.
