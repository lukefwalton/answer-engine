...
| 14 | (build) the corpus and vectors are produced in this session | Corpus bodies produced from live public-domain sources. Vectors PENDING `demo:build` | `nothing` (process) | Run `demo:build`, commit vectors, then run the gate |
