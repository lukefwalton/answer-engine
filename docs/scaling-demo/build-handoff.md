...
### Public ledger — George Adam Smith (theologian), dir `demo/corpus/public/george-adam-smith/`
...
- lives in `demo/corpus/synthetic/` (the quarantine is one flag) and carries a `synthetic: true` frontmatter marker (a second, in-file flag; the `PrivateNote` type does not read it, so it changes nothing the engine sees),
...
1. `npm run demo:run` (the `--natural` headline, no key needed once vectors are committed). Confirm: rank correlation FP-vs-int8 above the bar, and the full gold suite passes. Record the headline numbers in delta-log row 2 / 7.
...
3. Optional keyed bonus: `npm run demo:run -- --full` runs the answer-mode adjudication (related-material routes without restating). This exercises selection, not A2 — int8 never touches the answer model's confabulation residue.
